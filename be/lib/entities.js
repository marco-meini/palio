...
export async function getOrCreateCavallo(client, { sourceId, nome }) {
  const { rows } = await client.query(
    `INSERT INTO cavalli (source_id, nome)
     VALUES ($1, $2)
     ON CONFLICT (source_id) DO UPDATE SET nome = EXCLUDED.nome
     RETURNING id`,
    [sourceId, normalizeNome(nome)],
  );
  return rows[0].id;
}
 ...
 * @param {{ sourceId: string, nome: string, soprannome?: string|null }} row
 * @param {{ fullNome?: boolean }} [opts] fullNome: aggiorna nome da sommario (vincitrice)
 */
export async function getOrCreateFantino(client, { sourceId, nome, soprannome }, opts = {}) {
  ...
  const { rows } = await client.query(
    `INSERT INTO fantini (source_id, nome, soprannome) VALUES ($1, $2, $3) RETURNING id`,
    [sourceId, insertNome, s],
  );
  return rows[0].id;
}
...
export const getOrCreateCapitano = (client, nome) =>
  getOrCreateByNome(client, 'capitani', nome);

export const getOrCreateMangini = (client, nome) =>
  getOrCreateByNome(client, 'mangini', nome);

export const getOrCreateBarbaresco = (client, nome) =>
  getOrCreateByNome(client, 'barbareschi', nome);

export const getOrCreatePriore = (client, nome) =>
  getOrCreateByNome(client, 'priori', nome);
