...
function yearFilters(params) {
    ...
    if (params.yearFrom != null) {
        parts.push(`extract(year FROM p.data_palio)::int >= ${assertInt(params.yearFrom, 'yearFrom')}`);
    }
    if (params.yearTo != null) {
        parts.push(`extract(year FROM p.data_palio)::int <= ${assertInt(params.yearTo, 'yearTo')}`);
    }
    return parts.length ? `\n  AND ${parts.join('\n  AND ')}` : '';
}
const RECIPES = {
    same_horse_consecutive_cross_year: {
        description: 'Stesso cavallo (cavallo_id) per la stessa contrada in due palii consecutivi (data_palio, id) con anni diversi',
        validate() { },
        buildSql: () => `
WITH palii_ord AS (
  SELECT
...
export function validateRecipeParams(recipe, params = {}) {
    ...
    if (!RECIPE_NAMES.includes((recipe))) {
        throw new Error(`Ricetta sconosciuta: ${recipe}. Valide: ${RECIPE_NAMES.join(', ')}`);
    }
...
//# sourceMappingURL=palio-recipes.js.map