...
export function sourceCodeFromUrl(urlOrPath) {
    ...
    if (!m)
        throw new Error(`Cannot extract source code from URL: ${urlOrPath}`);
    return m[1];
}
/**
...
export function parsePalioPage(html, pageUrl) {
    ...
    const straordinario = /\bpalio\s+straordinari[oa]\b/.test(h1Text + noteText) ||
        /\bstraordinari[oa]\b/.test(h1Text);
    const combined = `${h1Text} ${noteText}`;
    const annullato = /\bpalio\s+(?:è\s+stato\s+)?annullat[oa]\b/i.test(combined) ||
...
export function parseOrdineEstrazione(html) {
    ...
    root.find('.Corniciato').each((_, cornEl) => {
        ...
        if (!/estrazione/i.test(lower) || /altre\s+sette/i.test(lower))
            return;
...
export function parseAssegnazioneCavalli(html) {
    ...
    root.find('.RigaTabCavalli:not(.IntestazioneTabCavalli)').each((_, rowEl) => {
        ...
        if (!Number.isInteger(ordineAssegnazione) || ordineAssegnazione < 1)
            return;
        ...
        if (!proprietarioCavallo || !cavalloPresoDa)
            return;
...
export function parseDirigenze(html) {
    ...
    root.find('.Corniciato.Riquadro').each((_, el) => {
        const box = $(el);
        const rawCode = extractOnclickCode(box.find('.OmbraBandiera').first(), 'DC') ||
...
function parseContradaBox($, box) {
    ...
    if (!contradaCode)
        return null;
    const contradaName = box.find('.RigaOrdCanape.PrimaRiga').first().text().trim() || nameFromCode(contradaCode);
    if (contradaName.includes('{{'))
        return null;
    if (isNonPartecipaLabel(label)) {
        return {
            contradaCode,
    ...
    const vincitriceDaCanape = fantinoEl.hasClass('VV') ||
...
function parseVintoDa($) {
    ...
    const { nome: fantinoNome, soprannome: fantinoSoprannome } = parseFantinoNominativo(cheerio.load(`<div>${fantinoHtml}</div>`)('div'));
    return {
        contradaCode,
        contradaName,
        cavallo: cavalloSourceId ? { sourceId: cavalloSourceId, nome: cavalloNome } : null,
...
function parseStaffParagraph(p) {
    ...
    const pick = (labels) => {
        for (const label of labels) {
            const re = new RegExp(`${label}:\\s*([^\\n]+?)(?=\\s*(?:Capitano|Governatore|Priore|Rettore|Mangini|Barbaresco):|$)`, 'i');
            const m = text.match(re);
            if (m)
                return stripLinkText(m[1]);
        }
        return null;
    };
...
function extractOnclickCode(el, fn) {
    ...
    if (!m)
        return null;
    let code = m[1];
    if (fn === 'DC' && code.startsWith('-'))
        code = code.slice(1);
    return code;
}
//# sourceMappingURL=ilpalio-parser.js.map