...
import { DEFAULT_READONLY_QUERY_TIMEOUT_MS, pgFindObjects, pgListTables, pgProfileTest, pgRunQuery, pgSchemaInspect, } from './pg-readonly-driver.js';
...
const FORBIDDEN_KEYWORD = /\b(INSERT|UPDATE|DELETE|DROP|ALTER|CREATE|TRUNCATE|GRANT|REVOKE|COMMIT|ROLLBACK|CALL|DO|COPY|LOCK|VACUUM|ANALYZE|REINDEX|CLUSTER|REFRESH|migration)\b/i;
...
export function assertReadOnlySql(sql) {
    ...
    for (const statement of executable) {
        ...
        if (FORBIDDEN_KEYWORD.test(cleaned)) {
            throw new Error('Query non consentita: sono permesse solo SELECT, WITH ed EXPLAIN in sola lettura');
        }
        if (!ALLOWED_START.test(cleaned)) {
            throw new Error('Query non consentita: la query deve iniziare con SELECT, WITH o EXPLAIN');
        }
    }
}
...
//# sourceMappingURL=postgres-cli.js.map