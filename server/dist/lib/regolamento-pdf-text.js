...
async function ocrPdfWithPdftoppm(pdfPath) {
    ...
    if (!pdftoppm) {
        throw new Error('pdftoppm non trovato (necessario per PDF scansionati). Installa poppler: brew install poppler');
    }
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'palio-regolamento-'));
    const prefix = path.join(tmpDir, 'page');
    try {
        execFileSync(pdftoppm, ['-png', '-r', '200', pdfPath, prefix], { stdio: ['ignore', 'pipe', 'pipe'] });
        const imagePaths = fs
            .readdirSync(tmpDir)
        ...
        if (err instanceof Error && err.message.startsWith('pdftoppm'))
            throw err;
...
export async function extractTextFromPdfFile(pdfPath) {
    ...
    console.info(`[index-regolamento] Testo PDF scarso (${direct.length} caratteri, ${parsed.numpages} pagine) — OCR in corso…`);
    return ocrPdfWithPdftoppm(pdfPath);
}
//# sourceMappingURL=regolamento-pdf-text.js.map