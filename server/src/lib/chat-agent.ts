...
export function trimMessagesForModel(messages: ModelMessage[]) {
  const { maxHistoryMessages, maxMessageChars } = settings();
  return messages.slice(-maxHistoryMessages).map((m) => {
    if (typeof m.content !== 'string') return m;
    if (m.content.length <= maxMessageChars) return m;
    return {
      ...m,
...
function countRegolamentoCalls(steps: StepResult<Record<string, Tool>>[]) {
  return steps
    .flatMap((step) => step.toolCalls ?? [])
    .filter((call) => call.toolName === 'search_regolamento').length;
}

export function streamPalioChat({
  messages,
  onToolStart,
  onToolEnd,
}: {
