...
export function parseIngressoCanape(html) {
  const $ = cheerio.load(html);
  /** @type {ReturnType<typeof parseContradaBox>[]} */
  const out = [];
 ...
 * @param {string} html
 * @returns {Map<string, { ordine: number, estratta: boolean, estrattaDaName?: string }>}
 */
export function parseOrdineEstrazione(html) {
  const $ = cheerio.load(html);
  const root = $('#sezPrincipale').length ? $('#sezPrincipale') : $('section').first();
  /** @type {Map<string, { ordine: number, estratta: boolean, estrattaDaName?: string }>} */
  const out = new Map();
 ...
 * @param {string} html
 * @returns {Map<string, { ordineAssegnazione: number, orecchio: number, coscia: number, proprietarioCavallo: string, cavalloPresoDa: string }>}
 */
export function parseAssegnazioneCavalli(html) {
  const $ = cheerio.load(html);
  const root = $('#sezPrincipale').length ? $('#sezPrincipale') : $('section').first();
  /** @type {Map<string, { ordineAssegnazione: number, orecchio: number, coscia: number, proprietarioCavallo: string, cavalloPresoDa: string }>} */
  const out = new Map();
 ...
 * @param {string} html
 * @returns {Map<string, { capitano?: string, priore?: string, barbaresco?: string, mangini: string[] }>}
 */
export function parseDirigenze(html) {
  const $ = cheerio.load(html);
  const root = $('#sezPrincipale').length ? $('#sezPrincipale') : $('section').first();
  /** @type {Map<string, { capitano?: string, priore?: string, barbaresco?: string, mangini: string[] }>} */
  const out = new Map();
...
/**
 * @param {cheerio.Cheerio<cheerio.Element>} box
 */
...
/**
 * @param {cheerio.Cheerio<cheerio.Element>} $
 */
...
/**
 * @param {cheerio.Cheerio<cheerio.Element>} p
 */
